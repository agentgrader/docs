...
| trace --stats | `agr trace --last --stats` shows token and cost statistics: total/avg/max tokens per step, cache hit rate, total cost; `--json` emits structured data; useful for understanding token usage patterns without reading the full trace |
| list --active | `agr list --active` shows only currently in-progress runs (status = running); combinable with all list filters; useful for detecting stuck/hung runs or checking if a bench is still running |
| bench --ci shorthand | `agr bench --ci` enables `--fail-on-failure`, `--show-failures`, and `--github-step-summary` in one flag; `agr init --ci` workflow updated to use `agr bench --suite tasks/ --ci` for cleaner CI configs |
